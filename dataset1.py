...
class ImbalanceCIFAR10(datasets.CIFAR100):
    ...
    def __init__(self, root, imb_type='exp', imb_factor=0.01, rand_number=0, train=True,
                 transform=None, target_transform=None, download=False,weighted_alpha = 1.0,shuffle_cls_idx = False,cutout_prob=-1.1):
        super(ImbalanceCIFAR10, self).__init__(root, train, transform, target_transform, download)
        np.random.seed(rand_number)
        random.seed(rand_number)
        self.shuffle_cls_idx = shuffle_cls_idx
    ...
    def get_img_num_per_cls(self, cls_num, imb_type, imb_factor):
        ...
        img_num_per_cls = []
        if imb_type == 'exp':
            ord_seq = [*range(cls_num)] # order sequence
            if self.shuffle_cls_idx:
                
                random.shuffle(ord_seq)
            for cls_idx in range(cls_num):

                #the index should be shuffled
                num_idx =  ord_seq[cls_idx]
                num = img_max * (imb_factor ** (num_idx / (cls_num - 1.0)))
